...
void PlikZUzytkownikami:: dopiszUzytkownikaDoPliku(Uzytkownik uzytkownik)
{
    ...
    {
        liniaZDanymiUzytkownika = zamienDaneUzytkownikaNaLinieZDanymiOddzielonymiPionowymiKreskami(uzytkownik);
...
void PlikZUzytkownikami::zapiszWszystkichUzytkownikowDoPliku(vector <Uzytkownik> &uzytkownicy)
{
    ...
    if (plikTekstowy.good() == true)
    {
        ...
        {
            liniaZDanymiUzytkownika = zamienDaneUzytkownikaNaLinieZDanymiOddzielonymiPionowymiKreskami(uzytkownicy[i]);

            if (i == uzytkownicy.size() - 1)  // Ostatni użytkownik
            {
               plikTekstowy << liniaZDanymiUzytkownika;
            }
            else
